...
internal sealed class UpdateMessageHandler : IRequestHandler<UpdateMessage, MessageAndChatModel>
{
    ...
    private static readonly ILogger Logger = Log.ForContext<UpdateMessageHandler>();
    private readonly IChatsRepository chatsRepository;
    private readonly IMessagesRepository messagesRepository;

    public async Task<MessageAndChatModel> Handle(UpdateMessage command, CancellationToken cancellationToken)
    {
        ...
        {
            Logger.Warning("User {ActorId} attempted to update message {MessageId} without being the author", command.ActorId, command.MessageId);
            throw new ApplicationException("Only the message author can update the message.");
        }
